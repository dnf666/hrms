...
public class GoodsReceivedNoteProductListHeaderController extends StandardListHeaderController {
    ...
    protected Result after(Arg arg, Result result) {
        ...
        log.debug("GoodsReceivedNoteProductListHeaderController, arg[{}], result[{}]", arg, result);
        //删除冗余字段：入库单产品Id和入库单编号
    ...
    private void delRedundancyFields(Result result, Layout layout) {
        try {
            ...
            fieldSection.get(0).setFields(formFields.stream().filter(formField ->
                !formField.getFieldName().equals(GoodsReceivedNoteProductConstants.Field.Name.apiName) &&
                        !formField.getFieldName().equals(GoodsReceivedNoteProductConstants.Field.GoodsReceivedNote.apiName)
