...
@Slf4j
public class AccountSignCertifyAddAction extends CommonAddAction {
    ...
    protected void before(Arg arg) {
        ...
        //查询客户是否已有认证记录
        String accountId = (String) arg.getObjectData().get(AccountSignCertifyObjConstants.Field.AccountId.apiName);
        if (accountSignCertifyObjManager.isAccountHasRecord(user, accountId)) {
            throw new ElecSignBusinessException(ElecSignErrorCode.BUSINESS_ERROR, "该客户已有相关记录");
        }
        ...
        arg.getObjectData().put(AccountSignCertifyObjConstants.Field.CertifyStatus.apiName, CertifyStatusEnum.NO_RECORD.getStatus());
        ...
        log.info("AccountSignCertifyAddAction before objectData[{}]", arg.getObjectData());
        super.before(arg);
    }
    ...
    private void checkHasUse(User user, Arg arg) {
        String regMobile = (String) arg.getObjectData().get(AccountSignCertifyObjConstants.Field.RegMobile.apiName);
        if (accountSignCertifyObjManager.hasRegMobileUsed(user, regMobile)) {
            throw new ElecSignBusinessException(ElecSignErrorCode.BUSINESS_ERROR, "注册手机号已被使用");
        }

        String enterpriseName = (String) arg.getObjectData().get(AccountSignCertifyObjConstants.Field.EnterpriseName.apiName);
        if (enterpriseName != null && accountSignCertifyObjManager.hasEnterpriseNameUsed(user, enterpriseName)) {
