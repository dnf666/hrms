...
public class RequisitionNoteProductListHeaderController extends StandardListHeaderController {
    ...
    protected Result after(Arg arg, Result result) {
        ...
        log.debug("RequisitionNoteProductListHeaderController, arg[{}], result[{}]", arg, result);
        //删除冗余字段：调拨单单产品Id和调拨单编号
    ...
    private void delRedundancyFields(Result result, Layout layout) {
        try {
            ...
            fieldSection.get(0).setFields(formFields.stream().filter(formField ->
                !formField.getFieldName().equals(RequisitionNoteProductConstants.Field.Name.apiName) &&
                        !formField.getFieldName().equals(RequisitionNoteProductConstants.Field.Requisition.apiName)
