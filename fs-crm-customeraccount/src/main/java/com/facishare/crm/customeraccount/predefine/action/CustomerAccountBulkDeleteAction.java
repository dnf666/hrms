...
public class CustomerAccountBulkDeleteAction extends StandardBulkDeleteAction {
    private CustomerAccountManager customerAccountManager;
    private CrmManager crmManager;

    @Override
    protected void before(StandardBulkDeleteAction.Arg arg) {
        super.before(arg);
        ...
        if (!RequestUtil.isFromInner(actionContext)) {
            ...
            throw new ValidateException(String.format("客户账户{%s}关联客户{%s}，请删除客户！", Joiner.on(",").join(cusANameCusNameMap.keySet()), Joiner.on(",").join(cusANameCusNameMap.values())));
        }
    }
