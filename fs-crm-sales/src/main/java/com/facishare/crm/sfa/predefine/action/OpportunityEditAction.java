 ...
 */
@Slf4j
public class OpportunityEditAction extends SFAEditAction {
    ...
    protected Result doAct(Arg arg) {
        log.info("OpportunityEditAction>act()arg=" + JsonUtil.toJsonWithNullValues(arg));
        super.doAct(arg);

        IObjectData updated = serviceFacade.updateObjectData(actionContext.getUser(), objectData);
        return Result.builder().objectData(ObjectDataDocument.of(updated)).build();
    }

    @Override
    protected Result after(Arg arg, Result result) {
        log.info("OpportunityEditAction>after()arg=" + JsonUtil.toJsonWithNullValues(arg));
        log.info("OpportunityEditAction>after()result=" + JsonUtil.toJsonWithNullValues(result));
