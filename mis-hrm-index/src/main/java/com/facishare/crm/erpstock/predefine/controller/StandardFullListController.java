 ...
 */
@Slf4j
public class StandardFullListController extends StandardListController {
    @Override
    protected BaseListController.Result doService(StandardListController.Arg arg) {
        BaseListController.Result r = super.doService(arg);
        if (null != r && CollectionUtils.isNotEmpty(r.getDataList())) {
            r.getDataList().forEach(d -> fillWithDetails(controllerContext.getRequestContext(), controllerContext.getObjectApiName(), d));
        }
        return r;
    }
    ...
    private ObjectDataDocument fillWithDetails(RequestContext context, String describeApiName,
        ...
        List<IObjectDescribe> detailDescribeList = serviceFacade.findDetailDescribes(context.getTenantId(), describeApiName);
        Map<String, List<IObjectData>> details = serviceFacade.findDetailObjectDataList(detailDescribeList, data.toObjectData(), context.getUser());
