...
@Slf4j
public class InternalSignCertifyEditAction extends CommonEditAction {
    ...
    protected void before(Arg arg) {
        ...
        // 校验认证状态
        String certifyStatus = objectData.get(InternalSignCertifyObjConstants.Field.CertifyStatus.apiName, String.class);
    ...
    private void checkHasUse(User user, Arg arg) {
        ...
        String currentRegMobile = (String) objectData.get(InternalSignCertifyObjConstants.Field.RegMobile.apiName);
        String newRegMobile = (String) arg.getObjectData().get(InternalSignCertifyObjConstants.Field.RegMobile.apiName);
        if (!Objects.equals(currentRegMobile, newRegMobile)) {
            if (internalSignCertifyObjManager.hasRegMobileUsed(user, newRegMobile)) {
                throw new ElecSignBusinessException(ElecSignErrorCode.BUSINESS_ERROR, "注册手机号已被使用");
            }
        }

        //'企业名称'在同个租户中是否已被使用
        String currentEnterpriseName = (String) objectData.get(InternalSignCertifyObjConstants.Field.EnterpriseName.apiName);
        String newEnterpriseName = (String) arg.getObjectData().get(InternalSignCertifyObjConstants.Field.EnterpriseName.apiName);
        if (!Objects.equals(currentEnterpriseName, newEnterpriseName)) {
            if (internalSignCertifyObjManager.hasEnterpriseNameUsed(user, newEnterpriseName)) {
