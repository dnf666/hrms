...
//    private static final String FOOT_TXT = "电子签约专用章";// 底部文字（不要用了）
    ...
    public static void main(String[] args) throws IOException {
        byte[] data = genSealData("深圳市发现科技有限公司");
//        byte[] data = genSealData("山东xxxxxx有限公司合肥分xx有公司合肥分公司");
        System.out.println("深圳市发现科技有限公司有限公有限公司司有限公司".length());
        System.out.println("山东xxxxxx有限公司合肥分xx有公司合肥分公司".length());

        OutputStream outputStream = new FileOutputStream("D:\\test2.png");
    ...
    private static void drwaHeadText(String companyName, Graphics2D g2d, int circleRadius) {
        ...
        int headFontSize;
        if (companyNameLen < 10) { // 字体根据画布大小微调出来的
            headFontSize = 47;
        }
        else if (10 <= companyNameLen  &&  companyNameLen < 20) {   //长度10：用37还可以    长度19: 用28还可以
            headFontSize = 37 - (companyNameLen - 10);
        }
        else if (20 <= companyNameLen  &&  companyNameLen < 30) {   //长度20: 用28还可以    长度29：用19还可以
            headFontSize = 28 - (companyNameLen - 20);
        }
        else if (30 <= companyNameLen  &&  companyNameLen < 40) {   //长度30: 用19还可以    长度39：用15还可以
            headFontSize = 19 - (companyNameLen - 30) / 2;
        }
        else {
            headFontSize = 19 - (companyNameLen - 30) / 3;
        }
        log.info("drwaHeadText, companyName[{}], companyNameLen[{}], headFontSize[{}]", companyName, companyNameLen, headFontSize);
