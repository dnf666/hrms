...
public class PromotionInvalidAction extends StandardInvalidAction {
    ...
    protected void before(Arg arg) {
        ...
        super.before(arg);
        Map<String, String> idNameMap = objectDataList.stream().filter(objectData -> objectData.getId().equals(arg.getObjectDataId())).collect(Collectors.toMap(objectData -> objectData.getId(), IObjectData::getName));
        ...
        if (CollectionUtils.isNotEmpty(relatedPromotionVos)) {
            ...
            for (Map.Entry<Integer, List<ObjectRelatedPromotionModel.RelatedPromotionVo>> entry : objectTypeVoMap.entrySet()) {//(obType, relatedVos)
